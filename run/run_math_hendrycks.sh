#!/bin/bash
# Random-selection AL sweep on Hendrycks MATH (competition math).
#
# Prereq (one-time): preprocess MATH into cache/math_hendrycks_preprocessed/
#     python -m atgen.utils.data.preprocess_math --out cache/math_hendrycks_preprocessed
#
# Sweeps query_size x seed for `al=random` on `data=math_hendrycks`, distributing
...
#
# Aggregate after:
#   python -m atgen.utils.aggregate_metrics outputs/$(date +%F)/*_math_hendrycks_random_[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]
#
# Headline metric is `exact_match_math` (extracts \boxed{...} from pred & ref;
# see compute_metrics._extract_math_answer).
...
EXP_NAME=math_hendrycks_random

query_sizes=(50 100 250 1000 2500 5000)
seeds=(42 1234567 31337)

LOG_DIR=run/_math_hendrycks_logs/$(date +%F_%H-%M-%S)
mkdir -p "$LOG_DIR"
...
#   - greedy decoding (deterministic, math-friendly)
