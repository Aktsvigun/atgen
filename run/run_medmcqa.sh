#!/bin/bash
# Random-selection AL sweep on MedMCQA (medical 4-option MCQ).
#
# Prereq (one-time):
#   python -m atgen.utils.data.preprocess_medmcqa --out cache/medmcqa_preprocessed
#
# Sweeps query_size x seed for `al=random` on `data=medmcqa`, distributing
...
#
# Aggregate after:
#   python -m atgen.utils.aggregate_metrics outputs/$(date +%F)/*_medmcqa_random_[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]
#
# Headline metric is `exact_match` (single-letter A/B/C/D).
...
EXP_NAME=medmcqa_random

query_sizes=(100 500 2500 10000)
seeds=(42 1234567 31337)

LOG_DIR=run/_medmcqa_logs/$(date +%F_%H-%M-%S)
mkdir -p "$LOG_DIR"
